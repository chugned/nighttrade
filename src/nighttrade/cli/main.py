...
from ..market_hours import describe as describe_market, next_market_open, session_at
...
def _mock_candles(config, n_bars: int, drift: float, volatility: float):
    ...
    return generate_random_walk(
        symbol=config.symbol, n_bars=n_bars, start_price=230.0,
...
def sandbox_check(
    ...
    mode = ("broker paper account connected" if client
            else "local paper simulation (no broker)")
...
def observe(
    ...
    live: bool = typer.Option(
        False, "--live", help="Use REAL stock data (yfinance) instead of the "
    ...
    if live:
        ...
        _console.print(f"Live mode: fetching real intraday data for "
        ...
        if not available:
            _console.print("[bold red]No live data available[/bold red] — "
                            "market data could not be fetched. Aborting.")
            raise typer.Exit(code=1)
        watchlist_cfg = WatchlistConfig(symbols=available)
        _console.print(f"[green]Live feed ready[/green] — "
                        f"{len(available)}/{len(universe)} symbols have data.")
    else:
        feed = LiveMockFeed()
        watchlist_cfg = load_watchlist_config()

    observer = Observer(cfg, watchlist_cfg,
                        db=ObservatoryDB(), feed=feed, model=model)
    _console.print(f"Observing {len(observer.watchlist_config.symbols)} symbols "
...
def dashboard(
    ...
    uvicorn.run("nighttrade.dashboard.app:app", host=host, port=port,
                log_level="warning")
...
def rank(
    ...
    if live:
        ...
        universe = liquid_universe(limit=max_symbols)
        _console.print(f"Fetching live data for {len(universe)} S&P 500 stocks "
                        f"[dim](first download takes a minute)…[/dim]")
        feed = YFinanceFeed(universe)
        feed.refresh_now()
    ...
    _console.print(f"Ranked [bold]{len(ranked.stocks)}[/bold] stocks "
    ...
    def _table(title: str, rows, style: str) -> None:
        ...
        for s in rows:
            fz = s.factor_z
            cells = [str(s.rank), s.symbol, f"{s.price:,.2f}",
                     f"{s.composite_z:+.2f}", f"{fz['momentum']:+.2f}",
                     f"{fz['trend']:+.2f}", f"{fz['reversion']:+.2f}",
                     f"{fz['low_vol']:+.2f}"]
            if has_ml:
    ...
    _table(f"Top {top} — strongest relative to the universe",
           ranked.top(top), "green")
    _console.print()
    _table(f"Bottom {top} — weakest relative to the universe",
           ranked.bottom(top), "red")
    _console.print(
...
def market_hours() -> None:
    ...
    _console.print(table)
    _console.print("[dim]Paper trades are placed only during the REGULAR "
                    "session.[/dim]")
