...
_ALLOWED_HOSTS = frozenset(urllib.parse.urlparse(u).hostname for u in _PAPER_URLS.values())
...
class SandboxExchangeClient:
    ...
    def _request(self, method: str, path: str, body: Optional[dict] = None) -> dict:
        ...
        with httpx.Client(base_url=self.base_url, timeout=self._timeout) as client:
            resp = client.request(method, path, headers=self._headers(), content=content)
    ...
    def verify_credentials(self) -> ApiKeyPermissions:
        ...
        try:
            ...
            raise ExchangeError(f"sandbox: could not reach {self.broker} paper API: {exc}") from exc

        # The single, non-negotiable safety gate.
        enforce_key_safety(permissions, self._config)
        self._creds = ApiCredentials(
            broker=self._creds.broker,
            api_key=self._creds.api_key,
            api_secret=self._creds.api_secret,
            permissions=permissions,
        )
        _log.info("sandbox key verified (%s paper): scopes=%s", self.broker, permissions.describe())
        return permissions

    def _read_permissions(self) -> ApiKeyPermissions:
        account = self._request("GET", "/v2/account")
        blocked = bool(account.get("account_blocked")) or bool(account.get("trading_blocked"))
    ...
    @staticmethod
    def _fill_from_alpaca(symbol: str, side: Side, quantity: float, resp: dict) -> Fill:
        ...
        return Fill(
            order_id=str(resp.get("id", "paper")),
            symbol=symbol,
            side=side,
            quantity=filled_qty,
            price=avg_price,
...
def build_sandbox_client(config: AppConfig) -> Optional[SandboxExchangeClient]:
    ...
    if not config.runtime.allow_network:
        _log.warning(
            "sandbox enabled but runtime.allow_network is false — "
    ...
    if credentials is None:
        _log.warning(
            "sandbox enabled but no %s paper keys configured — " "staying in paper-simulation mode",
            sb.broker,
        )
        return None
