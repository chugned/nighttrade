"""Sandbox broker — paper bookkeeping with optional broker paper-account execution.
...
* ``simulated`` — orders are filled by the local simulator. This is the
  default and the only mode reachable with a read-only paper key.
* ``broker_paper`` — orders are placed on the broker's PAPER account
  (simulated shares at the broker). Reachable only with a verified
  order-scoped paper key *and* ``sandbox.require_read_only_keys: false``.
...
class SandboxBroker:
    ...
    def execution_mode(self) -> str:
        """``'broker_paper'`` if paper orders will be placed, else ``'simulated'``."""
    ...
    @property
    def is_broker_paper_execution(self) -> bool:
        return self.execution_mode == "broker_paper"
    ...
    def submit_market_order(
        ...
        """Execute an order — on the broker paper account if enabled, else simulated.
    ...
    def paper_balances(self) -> Dict[str, float]:
        """Read balances from the broker paper account (if connected)."""
        if self._client is None:
            return {}
