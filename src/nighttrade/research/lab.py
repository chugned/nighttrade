...
class ResearchLab:
    ...
    def __init__(self, config: AppConfig,
                 cache: Optional[HistoryCache] = None) -> None:
        self.config = config
    ...
    def run(
        ...
        for symbol in symbols:
            candles = ((candles_by_symbol or {}).get(symbol)
                       or self.cache.get(symbol, years=years))
            ...
            try:
                ...
                skipped.append(symbol)
                continue
            results.append(SymbolResult(symbol, len(candles),
    ...
    def _aggregate(self, results: List[SymbolResult],
                   skipped: List[str]) -> ResearchReport:
        if not results:
            return ResearchReport(
                skipped=skipped, verdict="NO DATA",
        ...
        report_kwargs = dict(
            ...
            mean_overfit_gap=_mean([r.walkforward.mean_overfit_gap
                                    for r in with_folds]),
    ...
    def _verdict(self, agg: dict,
                 with_folds: List[SymbolResult]) -> "tuple[str, List[str]]":
        ...
        if not with_folds:
            return "INSUFFICIENT DATA", notes + [
                "Not enough history for a walk-forward split — no edge claim "
                "can be made."]
    ...
    def _with_stops(self, stop_mult: float, target_mult: float) -> AppConfig:
        ...
        fusion = self.config.fusion.model_copy(update={
            "stop_vol_mult": stop_mult, "target_vol_mult": target_mult})
        return self.config.model_copy(update={"fusion": fusion})

    @staticmethod
    def _agg_backtest(cfg: AppConfig,
                      series: List[List[OHLCV]]) -> "tuple[float, float, int]":
    ...
    def sweep_stops(
        ...
        if not in_sample:
            return SweepReport(notes=["No symbol had enough history for an "
        ...
        oos_ret, oos_win, oos_trades = self._agg_backtest(
            self._with_stops(best.stop_mult, best.stop_mult * best.reward_risk),
            out_sample)
        base_oos_ret, _, _ = self._agg_backtest(self.config, out_sample)

        notes = ["Backtests are optimistic — treat every number as an upper "
        ...
        return SweepReport(grid=grid, best=best, baseline=baseline,
                           oos_return=oos_ret, oos_win_rate=oos_win,
                           oos_trades=oos_trades,
                           baseline_oos_return=base_oos_ret, notes=notes)
    ...
    def evaluate_meta(
        ...
        notes = ["Backtests are optimistic — out-of-sample precision is the "
                 "only number here worth any trust."]
        if not x_tr:
            return MetaReport(notes=notes + ["Not enough real history to "
                                             "evaluate the meta-model."])

        model = MetaModel(self.config.runtime.random_seed).fit(
            pd.concat(x_tr, ignore_index=True),
            pd.concat(y_tr, ignore_index=True))
        x_test = pd.concat(x_te, ignore_index=True)
        y_test = pd.concat(y_te, ignore_index=True).to_numpy(dtype=int)
        if not model.is_trained:
            return MetaReport(train_samples=len(y_test), notes=notes + [
                "The meta-model could not train (single class / too few rows)."])
        ...
        if int(accepted.sum()) < 20:
            notes.append(f"Only {int(accepted.sum())} test trades cleared the "
        ...
        return MetaReport(train_samples=int(model.samples),
                          test_samples=int(len(y_test)), base_rate=base_rate,
                          accuracy=accuracy, precision=precision,
                          coverage=coverage, lift=lift, notes=notes)
