...
def test_backtest_is_deterministic(config):
    ...
    series = generate_random_walk(
        "AAPL", n_bars=130, start_price=30_000.0, drift=0.0008, volatility=0.004, seed=21
    )
...
def test_time_stop_is_a_third_barrier():
    ...
    trade = _OpenTrade(entry_price=100.0, stop=90.0, target=120.0, quantity=1.0, bar_opened=0)
    ts = datetime(2026, 1, 2, tzinfo=timezone.utc)
    quiet = OHLCV(
        symbol="AAPL", timestamp=ts, open=101, high=103, low=99, close=102
    )  # touches neither stop nor target

    # Not yet at the time barrier — no exit.
    assert Backtester._exit_price(quiet, trade, held_bars=5, time_stop_bars=20) is None
    # At/after the time barrier — force-close at the bar close.
    assert Backtester._exit_price(quiet, trade, held_bars=20, time_stop_bars=20) == 102
    # time_stop_bars=0 disables the barrier entirely.
    assert Backtester._exit_price(quiet, trade, held_bars=999, time_stop_bars=0) is None
    # The stop still takes priority over the time barrier.
    hit = OHLCV(symbol="AAPL", timestamp=ts, open=95, high=96, low=89, close=92)
    assert Backtester._exit_price(hit, trade, held_bars=99, time_stop_bars=20) == 90.0
...
def test_pipeline_decision_levels_consistent(uptrend_candles, config):
    book = build_orderbook("AAPL", uptrend_candles[-1].close, imbalance=0.5, jitter=0.0)
    result = AnalysisPipeline(config).analyze(
