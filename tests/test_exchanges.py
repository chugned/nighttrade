...
from nighttrade.exchanges import (
    MockExchange,
    build_provider,
    compute_consensus,
...
from nighttrade.models import ExchangeStatus, PriceTick
...
def test_mock_exchange_ticker_and_ohlcv():
    candles = generate_random_walk("AAPL", n_bars=100, seed=1)
    ex = MockExchange(candles)
    tick = ex.get_ticker("AAPL")
    assert tick.price > 0
    assert len(ex.get_ohlcv("AAPL", limit=50)) == 50


def test_mock_exchange_orderbook_uncrossed():
    candles = generate_random_walk("AAPL", n_bars=50, seed=1)
    book = MockExchange(candles).get_orderbook("AAPL", depth=10)
...
def test_provider_offline_consensus():
    ...
    cp = provider.get_consensus_price("AAPL")
    assert cp.price > 0
