...
def _candle(i: int, low: float, high: float, close: float,
            volume: float = 1000.0) -> OHLCV:
    return OHLCV(symbol="AAPL", timestamp=_T0 + timedelta(minutes=i),
                 open=round((low + high) / 2, 2), high=high, low=low,
...
def test_detect_halt_on_frozen_tape():
    frozen = [
        OHLCV(symbol="AAPL", timestamp=_T0 + timedelta(minutes=i),
...
def test_microstructure_thin_liquidity_on_low_rvol():
    ...
    sig = MicrostructureEngine().compute(
        build_orderbook("AAPL", 100.0, jitter=0.0), quiet)
