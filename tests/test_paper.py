...
def test_buy_opens_position_and_spends_cash():
    broker = _broker()
    broker.submit_market_order("o1", "AAPL", Side.BUY, 1.0,
...
def test_sell_without_position_raises():
    ...
    with pytest.raises(ValueError):
        broker.submit_market_order("o1", "AAPL", Side.SELL, 1.0, 30_000.0,
                                   100.0, _CFG.risk, _T0)


def test_round_trip_records_trade():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0,
                               1000.0, _CFG.risk, _T0)
    broker.submit_market_order("sell", "AAPL", Side.SELL, 1.0, 31_000.0,
...
def test_profitable_round_trip_has_positive_pnl():
    ...
    broker.submit_market_order("sell", "AAPL", Side.SELL, 1.0, 33_000.0,
...
def test_sell_quantity_clamped_to_holdings():
    ...
    fill = broker.submit_market_order("sell", "AAPL", Side.SELL, 999.0,
...
def test_fills_are_logged():
    broker = _broker()
    broker.submit_market_order("o1", "AAPL", Side.BUY, 1.0, 30_000.0,
                               1000.0, _CFG.risk, _T0)
