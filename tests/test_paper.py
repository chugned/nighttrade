...
def test_broker_starts_flat():
    ...
    assert not broker.has_position("AAPL")
...
def test_buy_opens_position_and_spends_cash():
    ...
    assert broker.has_position("AAPL")
    assert broker.cash < 100_000.0
...
def test_sell_without_position_raises():
    ...
    with pytest.raises(ValueError):
        broker.submit_market_order("o1", "AAPL", Side.SELL, 1.0, 30_000.0, 100.0, _CFG.risk, _T0)


def test_round_trip_records_trade():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0, 1000.0, _CFG.risk, _T0)
    broker.submit_market_order(
        "sell", "AAPL", Side.SELL, 1.0, 31_000.0, 1000.0, _CFG.risk, _T0 + timedelta(hours=1)
    )
...
def test_profitable_round_trip_has_positive_pnl():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0, 10_000.0, _CFG.risk, _T0)
    broker.submit_market_order(
        "sell", "AAPL", Side.SELL, 1.0, 33_000.0, 10_000.0, _CFG.risk, _T0 + timedelta(hours=1)
    )
...
def test_sell_quantity_clamped_to_holdings():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0, 1000.0, _CFG.risk, _T0)
    held = broker.position("AAPL").quantity
    fill = broker.submit_market_order(
        "sell", "AAPL", Side.SELL, 999.0, 30_000.0, 1000.0, _CFG.risk, _T0 + timedelta(hours=1)
    )
...
def test_equity_reflects_marked_position():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0, 10_000.0, _CFG.risk, _T0)
    eq_up = broker.equity({"AAPL": 35_000.0})
    eq_down = broker.equity({"AAPL": 25_000.0})
...
def test_snapshot_is_consistent():
    broker = _broker()
    broker.submit_market_order("buy", "AAPL", Side.BUY, 1.0, 30_000.0, 10_000.0, _CFG.risk, _T0)
    snap = broker.snapshot(_T0, {"AAPL": 30_000.0})
    assert "AAPL" in snap.open_symbols
    assert snap.equity == pytest.approx(broker.equity({"AAPL": 30_000.0}))


def test_fills_are_logged():
    broker = _broker()
    broker.submit_market_order("o1", "AAPL", Side.BUY, 1.0, 30_000.0, 1000.0, _CFG.risk, _T0)
