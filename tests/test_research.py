...
def test_walk_forward_is_purged(config):
    ...
    candles = generate_random_walk(
        "AAPL", n_bars=1200, start_price=200.0, drift=0.0002, volatility=0.005, seed=7
    )
...
def test_research_lab_produces_honest_report(config):
    ...
    candles_by_symbol = {
        sym: generate_random_walk(
            sym, n_bars=600, start_price=200.0, drift=0.0, volatility=0.006, seed=s
        )
        for s, sym in enumerate(["AAA", "BBB", "CCC"])
    }
    report = ResearchLab(config).run(list(candles_by_symbol), candles_by_symbol=candles_by_symbol)
    ...
    assert (
        "EDGE" not in report.verdict or "UNPROVEN" in report.verdict or report.verdict == "NO EDGE"
    )
...
def test_research_lab_skips_thin_history(config):
    ...
    report = ResearchLab(config).run(["TINY"], candles_by_symbol={"TINY": _daily("TINY", 40)})
...
def test_sweep_stops_produces_report(config):
    ...
    candles_by_symbol = {
        sym: generate_random_walk(
            sym, n_bars=900, start_price=200.0, drift=0.0003, volatility=0.006, seed=s
        )
        for s, sym in enumerate(["AAA", "BBB", "CCC"])
    }
...
def test_evaluate_meta_produces_report(config):
    ...
    candles_by_symbol = {
        sym: generate_random_walk(
            sym, n_bars=900, start_price=200.0, drift=0.0004, volatility=0.006, seed=s
        )
        for s, sym in enumerate(["AAA", "BBB", "CCC"])
    }
